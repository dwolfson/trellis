...
SPECS: list[SurveyDefSpec] = [
    ...
    SurveyDefSpec(
        ...
        description=(
            ...
            "— SQL view dependency/lineage analysis (sql_analysis), and the "
            "zero-fetch derivation tier over already-stored rows — "
            "classification, relationship graph, grain, fingerprint, schema "
            "conventions, change rates and a proposed DataScope (db_derived, "
            "Phase 1 slice 9). Prefixed by postgres_schema_and_stats because "
            "postgres_operations' activity-signal roll-up, sql_analysis's view "
            "analysis, and db_derived's own reads all depend on the rows it "
            "writes. Regenerated by scripts/generate_database_survey_definition.py."
        ),
        step_keys=[
            "postgres_schema_and_stats", "postgres_operations", "sql_analysis",
            "db_derived",
        ],
