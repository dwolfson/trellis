"""Projection must actually project — or the dead dial must stay unbuilt.
...
The mechanism is sound; its input is not. `project_rows` walks `parent_path`,
...
That is why the depth control was withdrawn rather than shipped: it would have
been a dial wired to nothing.
"""
...
def test_depth_control_stays_unbuilt_while_parents_are_orphans():
