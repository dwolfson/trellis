...
def test_repair_goes_through_heal_batch_so_the_reconciler_cannot_be_skipped(tmp_path):
    ...
    """
    batch = _batch(tmp_path, **{"a.md": _LINEAR, "b.md": _LINEAR})
    batch.post_heal = {"script": "scripts/reconcile.py"}
...
def test_repair_reports_a_failed_heal_rather_than_claiming_success(tmp_path):
    batch = _batch(tmp_path, **{"a.md": _LINEAR})
    with patch("resource_explorer.bootstrap.discover_batches", return_value=[batch]), \
         patch("resource_explorer.bootstrap.heal_batch",
...
def _batch(tmp_path, **docs):
    ...
    return type("B", (), {"batch_id": "survey-definitions", "path": tmp_path,
                          "files": list(docs), "post_heal": None})()


_LINEAR = "## Link Next Process Step\n### Guard\nAny\n\n### Guard\nAny\n"
