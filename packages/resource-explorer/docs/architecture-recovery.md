...
>
> **2026-09-03: §5 (Extraction design) moved out to its own document**,
> `architecture-recovery-extraction-design.md` — see the stub left in its place, §5 below, for
> why.
>
...
>
> **Revision note.** The RE-side claims added in the post-review revision — §5.5, §5.6, §5.7 (now in
> `architecture-recovery-extraction-design.md`), §6.0, §6.5, and the storage decisions in that
> document's §5.4 — *were* verified against `trellis/packages/resource-explorer`
...
>   produces byte-identical output. **If this feature's projection layer (§5.4 in
>   `architecture-recovery-extraction-design.md`; §6.1, §6.4 here) writes
...
>   that bug. Directly relevant to `architecture-recovery-extraction-design.md` §5.7: the proposed `repo_arch_detect` / `repo_code_metrics` /
...
| `Authoritative` | "comes from an authoritative source; the best set of values" | human-curated **and** declared sources (`architecture-recovery-extraction-design.md` §5.1) — genuinely the same epistemic status |
...
**Consequence: `architecture-recovery-extraction-design.md` §5.4's `derivation` field is redundant and is removed.** It was a local reinvention of
...
here, which is why `architecture-recovery-extraction-design.md` §5.4 keeps them RE-side.
...
**Moved to `architecture-recovery-extraction-design.md`** (2026-09-03) — this section had grown to
913 lines, a third of this document, and stood on its own: detectors, distillation, the
Architecture IR, evidence and confidence as first-class, partition validation (five subsections,
5.5a–5.5f, covering documentation, repo classification, user feedback, motivation, and the
black-box/white-box lens), tooling choices, and how extraction wraps as survey steps. Everything
downstream in this document (§6 onward) consumes *what* extraction produces — the IR, confidence,
components — not how it's produced, so nothing else here changed; see CLAUDE.md's "Writing
project docs" rule for why this was split out.

Subsections, unchanged, in the new document: §5.1 Detectors first, call graphs last · §5.2
Distillation — the noise reducer · §5.3 The Architecture IR · §5.4 Evidence and confidence are
first-class · §5.5 Validating the partition — three independent signals (with 5.5a–5.5f) · §5.6
Tooling — what to adopt, and what it costs · §5.7 Wrapping as survey steps.
...
new `architecture-recovery-extraction-design.md` §5.7 steps, which are all `target_shape: corpus`.
...
- **`scope_locator` is the universal join key** — evidence (`architecture-recovery-extraction-design.md` §5.4), metrics, findings, and the Egeria
...
- **`confidence` is already there** — `architecture-recovery-extraction-design.md` §5.4's per-claim confidence requirement is satisfied by the base
...
**(a) Inside extraction (`architecture-recovery-extraction-design.md` §5.2).** Naming, classification, merge adjudication, and drafting a
component's `description`. Bounded by the standing rule: *the LLM never invents a component with no
detector evidence behind it.* Cost-gated per `architecture-recovery-extraction-design.md` §5.6 — invoked only below a confidence threshold.
...
- Repos with no release discipline need a fallback. **Resolved (Q10): a precedence chain**, taking the
  ...
  2. **Published package version** — PyPI, npm, Maven. Often present where git tags are not, and Syft
     (`architecture-recovery-extraction-design.md` §5.6) surfaces it as a side effect of SBOM generation rather than as extra work.
...
- **It largely pre-answers §8.2a's divergence RFA.** The 30 divergent files are not an open question
  ...
  looked.
- **This is a *declared architecture* source that `architecture-recovery-extraction-design.md` §5.1's table cannot consume.** It is prose, not a
  manifest — detector-invisible, but squarely readable by the LLM. So distillation's remit (`architecture-recovery-extraction-design.md` §5.2) should
...
Revised after the extraction-design tooling pass (§5.5–5.7, in
`architecture-recovery-extraction-design.md`) and §6.0's simplification. Two prerequisites moved earlier;
...
partition); and `egeria` for scale. Compare the three `architecture-recovery-extraction-design.md` §5.5
...
Secondary, independent of the result: time `scc` and `lizard` to test `architecture-recovery-extraction-design.md` §5.6's Discovery-tier cost claim.
...
- **`git_clone_root` `ResourceProvider`** (treeless clone) alongside `_acquire_zipball_root` — without
  it Q11's history metrics have no data source (`architecture-recovery-extraction-design.md` §5.7 gap 2).
- **Binary provisioning** — pin `scc`, `ast-grep`, `syft`, `trivy` into the RE image with a version
  probe feeding `analyzerVersion` (`architecture-recovery-extraction-design.md` §5.7 gap 3).

### Phase 1 — Detectors + IR, no Egeria writes
Implement `architecture-recovery-extraction-design.md` §5.1's detector table **as ast-grep rules plus config parsers**, the §5.3 IR, and §5.4's
evidence into `project_analysis_findings`. Declared-architecture sources (`architecture-recovery-extraction-design.md` §5.1) short-circuit inference
where present. Distillation heuristics only — no LLM yet. Add the `architecture-recovery-extraction-design.md` §5.7 steps to `STEP_REGISTRY` and
...
Add LLM naming/classification within `architecture-recovery-extraction-design.md` §5.2's rule (never invents components) and §5.6's cost gate (only
...
framework adoption (`architecture-recovery-extraction-design.md` §5.6).
...
- ~~**Q4** — where do evidence and confidence live?~~ **RESOLVED, in two halves.**
  *Confidence:* the `Confidence` classification, confirmed against `Referenceable`
  ...
  carries the analyzer id, `steward` carries the curator. `architecture-recovery-extraction-design.md` §5.4's proposed `derivation` field was a
  reinvention of `confidenceLevel` and has been removed.
  *Evidence:* stays **RE-side** in `project_analysis_findings` (`architecture-recovery-extraction-design.md` §5.4), which already has the exact shape
...
- ~~**Q11** — the sibling annotation type for repository history?~~ **RESOLVED in principle:** we define
  ...
  **But it has a real prerequisite:** git history is not in a zipball, so it needs the `git_clone_root`
  resource provider (`architecture-recovery-extraction-design.md` §5.7 gap 2). Not optional if §6's value case is to be met in full.
...
- **Q13** — which §6.2 metrics justify *typed* properties versus `additionalProperties`. Deferred to
  after Phase 0, as before — but note `architecture-recovery-extraction-design.md` §5.6's tool choices (`scc`, `lizard`) make multi-language
...
1. **Distillation (`architecture-recovery-extraction-design.md` §5.2, Phase 5)** — blocks Scenario B entirely and every catalogue-facing
...
4. **Disposition (`architecture-recovery-extraction-design.md` §5.5d)** — check `ResourceUse` before inventing. Turns findings into an answer.
5. **Motivation (`architecture-recovery-extraction-design.md` §5.5d) and the derived lens (§5.5e)** — selects question sets; the lens is nearly
