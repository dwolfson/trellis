...
**Status: ON HOLD (2026-08-17).** Design complete; case 4 built and live-verified at the code/routing level (see below), but every native-survey path is currently blocked by a confirmed Egeria **server-side** bug, independently reproduced by hand: `PYEGERIA_ISSUES.md` ISSUE-51 — template-based asset creation (`create_postgres_server_element_from_template`, `create_folder_element_from_template`, likely all of that family) never wires up a working `Connection`, so any native survey against a template-created asset fails server-side with a `NullPointerException` ("connector is null"), regardless of resource type. This affects cases 1/2/4 (anything that needs a real Egeria-native survey to actually *complete*, not just be triggered) — case 3 (RE-local surveying) is entirely unaffected. **All work in this doc is paused until ISSUE-51 is fixed upstream.**
