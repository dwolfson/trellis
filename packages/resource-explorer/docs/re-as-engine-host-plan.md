...
Authoring markdown for a live probe target is ready: `docs/dr-egeria/re-delegated-step-probe.md` — a harmless `write-to-audit-log` request against the standard content pack's Stewardship engine (`RequestTypeDefinition.WRITE_AUDIT_LOG`, confirmed from the Java source, not live-checked). Running it will also be the first live verification of `Link Action to Action Executor` itself, flagged unverified in `egeria-python/CLAUDE.md` since 2026-07-15.

**Not yet done — blocked only on the local Egeria platform being up, not on any further design/code work**: run the probe markdown through Dr.Egeria, confirm `Link Action to Action Executor` resolves the "Stewardship" engine reference correctly (documented as unconfirmed in the probe markdown itself), trigger via `initiate_action_type_and_wait()`, confirm it reaches `COMPLETED` and produces the expected `ResourceMeasureAnnotation`. After that: wire a real `STEP_REGISTRY` entry (still needs a real target chosen for production use, separate from the throwaway probe).
