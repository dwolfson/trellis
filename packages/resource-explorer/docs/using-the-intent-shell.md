...
- **Schedules** — a flat, cross-analysis view of a selected resource's schedules, all in one table. Functionally the same data as the per-card ⏱ Schedule action above — use whichever view suits you: this page to see everything on one resource at once, or the per-card button when you're already looking at a specific analysis. **Read the caveat shown at the top of this pane** — not every listed analysis is fully implemented yet; cross-check against Discovery's Survey Definitions before relying on a scheduled run actually doing what its name suggests.
