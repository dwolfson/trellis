...
| TC-5 | Refactor `web/app.py` toward RE's router-per-domain factory pattern | in progress (slice 4/N done, 2026-09-07) | RE's `web/app.py` is 106 lines, 20 `include_router()` calls to one file per domain in `web/routes/`. EA's `web/app.py` was 2178 lines, 71 routes across 14 groups, only `admin.py` extracted. Most routes lean on shared module-level state a straight per-route move would duplicate or fork — moved that to `advisor/web/shared.py` first (`_get_rag()`'s singleton, `QueryRequest`/`FeedbackRequest`, `_intent_meta()`, the report-catalog helpers), so later slices have somewhere to import it from; `advisor.web.app` re-exports what its own remaining routes still use, since `advisor.rag_system` has a live external lazy import of `_intent_meta` from `advisor.web.app` specifically (verified still resolves, every slice). **Slices 1–3 (2026-09-07):** `shared.py` + `auth.py` (6 routes) → `feedback.py` (4) → `query.py` (2) + `plan_templates.py` (2) + `sessions.py` (2) + a **deliberately partial** `templates.py` (1 of 2 — `GET /api/templates/Column/fields` stayed behind, entangled with reports' not-yet-extracted `discover_draft_schema_internal`/`_SCHEMA_CACHE`; full reasoning in `git log` for each slice's commit). **Slice 4 (2026-09-07):** extracted `advisor/web/drafts.py` (5 routes: `GET /api/drafts`, `GET /api/drafts/{draft_id}`, `PATCH /api/drafts/{draft_id}/commands`, `DELETE /api/drafts/{draft_id}`, `POST /api/drafts/builder`) — fully self-contained, no `shared.py` dependency; the 5th route was physically separated in the original file (between `list_actions` and the deferred `Column/fields` route) but is the same "drafts" domain. `app.py`: 1447 → 1300 lines (2178 → 1814 → 1702 → 1447 → 1300 across four slices). Verified every slice the same way: `ruff check --select F` clean; `tests/unit/test_login_required_middleware.py` (13 tests) passing; full suite 315 passed every time (baseline match, 4 pre-existing unrelated failures deselected); live `TestClient` requests against every route in every new router — slice 4's included a full real create→get→delete cycle against `/api/drafts/builder`, confirming both the route wiring and that no stray draft file was left in the shared checkout afterward. One verification-method false alarm in slice 1 (resolved, not a real bug): naive `app.routes` enumeration makes `include_router()`-added routes look absent on this FastAPI version (lazy `_IncludedRouter` wrapper) — real HTTP requests are the check that holds. **Remaining slices, rising size/risk:** plans (19) → reports (23 + deferred `Column/fields` + `discover_draft_schema_internal`/`_SCHEMA_CACHE` — largest, most entangled, last). |
