...
| TC-5 | Refactor `web/app.py` toward RE's router-per-domain factory pattern | in progress (slice 3/N done, 2026-09-07) | RE's `web/app.py` is 106 lines — `lifespan` context manager, 20 `include_router()` calls to one file per domain in `web/routes/`, and a deliberate `/health` (liveness) vs. `/health/ready` (DB-exercising) split fixing a real prior incident. EA's `web/app.py` was 2178 lines, 71 routes (23 reports, 19 plans, 6 auth, 5 drafts, 4 feedback, 2 each templates/sessions/query/plan-templates, 1 each status/perspectives/egeria/actions/health/root), with only `admin.py` (499 lines, self-contained `APIRouter()`) actually extracted. Unlike `admin.py`, most of the remaining routes lean on shared module-level state a straight per-route move would duplicate or fork — `_get_rag()`'s `_rag` singleton, `QueryRequest`/`FeedbackRequest` models, `_INTENT_META`/`_intent_meta()`, the report-catalog helpers (`_topic_for`, `_spec_supported_formats`, `_catalog_formats`, `_is_runnable_spec`, `_load_report_catalog`). **Slice 1 (2026-09-07):** extracted `advisor/web/shared.py` (the cross-cutting state above) and `advisor/web/auth.py` (6 routes, no dependency on `shared.py`). **Slice 2 (2026-09-07):** extracted `advisor/web/feedback.py` (4 routes) — depends on `shared.py` for `FeedbackRequest`/`_extended_feedback_path` only. **Slice 3 (2026-09-07):** extracted `advisor/web/query.py` (2 routes: POST `/api/query`, POST `/api/query/stream` — depends on `_get_rag`/`QueryRequest`/`_intent_meta`), `advisor/web/plan_templates.py` (2 routes, self-contained), `advisor/web/sessions.py` (2 routes, self-contained), and a **deliberately partial** `advisor/web/templates.py` (1 of 2 routes: `GET /api/templates/{command_name}/fields` only). `GET /api/templates/Column/fields` stayed in `app.py` — it calls `discover_draft_schema_internal()`, reports-domain business logic (with its own `_SCHEMA_CACHE`) still living in app.py's not-yet-extracted reports section and also used by `GET /api/reports/drafts/{draft_id}/schema`; moving it now would mean either duplicating that logic or a smaller router reaching back into a bigger not-yet-extracted one, reversed from how every other slice has gone. Left for the reports slice, where `discover_draft_schema_internal` belongs. `_get_rag`/`QueryRequest` dropped from `app.py`'s re-export list (nothing left there uses them); `_intent_meta` kept with an explicit `# noqa: F401` — genuinely unused *in app.py* now but must stay importable there for `advisor.rag_system`'s external lazy import. `app.py`: 1702 → 1447 lines (2178 → 1814 → 1702 → 1447 across three slices). Verified all three slices: `ruff check --select F` clean on every touched/new file; `tests/unit/test_login_required_middleware.py` (13 tests) unchanged and passing; full suite 315 passed every time (matching the pre-change baseline, 4 pre-existing unrelated failures deselected); live `TestClient` requests against every route in every new router, including a genuine end-to-end `POST /api/query` (real pgvector search + embeddings + Ollama generation, 200) and a streamed `POST /api/query/stream` (SSE lines received), plus untouched neighbors (`/api/admin/status`, `/api/perspectives`, `/api/reports`, `/health`) all return the expected status codes. One verification-method false alarm during slice 1, resolved before concluding anything: naively enumerating `app.routes` makes `include_router()`-added routes look absent on *this* FastAPI version (wraps them in a lazy `_IncludedRouter`, confirmed by reproducing the same false-negative against the untouched original file) — real HTTP requests through `TestClient` are the check that actually holds. **Remaining slices, in rising order of size/risk:** drafts (5) → plans (19) → reports (23 + the deferred `Column/fields` route + `discover_draft_schema_internal`/`_SCHEMA_CACHE` — largest and most entangled, do last). |
