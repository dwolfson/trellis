...
| TC-5 | Refactor `web/app.py` toward RE's router-per-domain factory pattern | in progress (slice 5/N done, 2026-09-07) | RE's `web/app.py` is 106 lines, 20 `include_router()` calls to one file per domain in `web/routes/`. EA's `web/app.py` was 2178 lines, 71 routes across 14 groups, only `admin.py` extracted. Most routes lean on shared module-level state a straight per-route move would duplicate or fork — moved that to `advisor/web/shared.py` first (`_get_rag()`'s singleton, `QueryRequest`/`FeedbackRequest`, `_intent_meta()`, the report-catalog helpers), so later slices have somewhere to import it from; `advisor.web.app` re-exports what its own remaining routes still use, since `advisor.rag_system` has a live external lazy import of `_intent_meta` from `advisor.web.app` specifically (verified still resolves, every slice). **Slices 1–4 (2026-09-07):** `shared.py` + `auth.py` (6 routes) → `feedback.py` (4) → `query.py`+`plan_templates.py`+`sessions.py` (2 each) + a **deliberately partial** `templates.py` (1 of 2 — `GET /api/templates/Column/fields` stayed behind, entangled with reports' not-yet-extracted `discover_draft_schema_internal`/`_SCHEMA_CACHE`) → `drafts.py` (5, self-contained); full reasoning in `git log` for each slice's commit. **Slice 5 (2026-09-07):** extracted `advisor/web/plans.py` — all 19 plans routes, one contiguous block in the original file, fully self-contained (no `shared.py` dependency). Also dropped `asyncio`/`partial`/`datetime` from `app.py`'s top-level imports — all three became genuinely unused there once this slice moved out (the one `datetime` use remaining in app.py, a report-export route, already had its own local shadowing import). `app.py`: 1300 → 914 lines (2178 → 1814 → 1702 → 1447 → 1300 → 914 across five slices; under half the original size). Verified every slice the same way: `ruff check --select F` clean; `tests/unit/test_login_required_middleware.py` (13 tests) passing; full suite 315 passed every time (baseline match, 4 pre-existing unrelated failures deselected); live `TestClient` requests against every route in every new router. Slice 5's was the fullest lifecycle test yet: import → get → export → save (PUT) → versions → fork → save-as → save-as-template → delete/trash → restore-trash → delete → purge, on a real document, with every test artifact (the plan, its fork, its save-as copy, and the saved template) confirmed cleaned up afterward — nothing left in the shared checkout. One verification-method false alarm in slice 1 (resolved, not a real bug): naive `app.routes` enumeration makes `include_router()`-added routes look absent on this FastAPI version (lazy `_IncludedRouter` wrapper) — real HTTP requests are the check that holds. **Remaining, last slice:** reports (23 routes + the deferred `Column/fields` route + `discover_draft_schema_internal`/`_SCHEMA_CACHE` — largest and most entangled, deliberately saved for last). |
