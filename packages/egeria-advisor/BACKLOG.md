...
| TC-7 | Confirm `code_symbol_store.py`'s import from `ingest_to_milvus.py` is reachable | open | Found in `docs/ingestion-pipeline-audit.md`'s Java-symbol-extraction section — `advisor/ingest_to_milvus.py` is likely dead code post-Milvus-removal, but `code_symbol_store.py` is still imported there. Confirm whether that import path is ever actually exercised before removing/keeping either file. |
