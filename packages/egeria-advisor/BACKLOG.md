...
| TC-5 | Refactor `web/app.py` toward RE's router-per-domain factory pattern | in progress (slice 2/N done, 2026-09-07) | RE's `web/app.py` is 106 lines — `lifespan` context manager, 20 `include_router()` calls to one file per domain in `web/routes/`, and a deliberate `/health` (liveness) vs. `/health/ready` (DB-exercising) split fixing a real prior incident. EA's `web/app.py` was 2178 lines, 71 routes (23 reports, 19 plans, 6 auth, 5 drafts, 4 feedback, 2 each templates/sessions/query/plan-templates, 1 each status/perspectives/egeria/actions/health/root), with only `admin.py` (499 lines, self-contained `APIRouter()`) actually extracted. Unlike `admin.py`, most of the remaining routes lean on shared module-level state a straight per-route move would duplicate or fork — `_get_rag()`'s `_rag` singleton, `QueryRequest`/`FeedbackRequest` models, `_INTENT_META`/`_intent_meta()`, the report-catalog helpers (`_topic_for`, `_spec_supported_formats`, `_catalog_formats`, `_is_runnable_spec`, `_load_report_catalog`). **Slice 1 (2026-09-07):** extracted `advisor/web/shared.py` (the cross-cutting state above, so future slices have somewhere to import it from) and `advisor/web/auth.py` (6 routes, the cleanest boundary — no dependency on `shared.py` at all). `advisor.web.app` still re-exports everything `shared.py` holds that its own remaining routes use — `advisor.rag_system` has a live lazy import of `_intent_meta` from `advisor.web.app` specifically, verified still resolves. **Slice 2 (2026-09-07):** extracted `advisor/web/feedback.py` (4 routes: POST `/api/feedback`, GET `/api/feedback/extended`, PATCH `/api/feedback/extended/{idx}`, GET `/api/feedback/analysis`) — depends on `shared.py` for `FeedbackRequest`/`_extended_feedback_path`, nothing else; `FeedbackRequest`/`_extended_feedback_path` dropped from `app.py`'s re-export list since nothing there uses them anymore (checked by occurrence count before removing). `app.py` now 1702 lines (2178 → 1814 → 1702 across the two slices). Verified both slices: `ruff check --select F` clean on every touched/new file; `tests/unit/test_login_required_middleware.py` (13 tests) unchanged and passing; full suite 315 passed both times (matching the pre-change baseline, 4 pre-existing unrelated failures deselected); live `TestClient` requests against every route in both new routers (auth's 6, feedback's safely-exercisable 3 of 4 — see FB-1 below for the fourth) plus untouched neighbors (`/api/admin/status`, `/api/perspectives`, `/health`) all return the expected status codes. One verification-method false alarm during slice 1, resolved before concluding anything: naively enumerating `app.routes` makes `include_router()`-added routes look absent on *this* FastAPI version (wraps them in a lazy `_IncludedRouter`, confirmed by reproducing the same false-negative against the untouched original file) — real HTTP requests through `TestClient` are the check that actually holds. **Remaining slices, in rising order of size/risk:** templates/sessions/query/plan-templates (2 each) → drafts (5) → plans (19) → reports (23, largest and most entangled with the catalog helpers — do last). |
