...
| TC-5 | Refactor `web/app.py` toward RE's router-per-domain factory pattern | in progress (slice 1/N done, 2026-09-07) | RE's `web/app.py` is 106 lines — `lifespan` context manager, 20 `include_router()` calls to one file per domain in `web/routes/`, and a deliberate `/health` (liveness) vs. `/health/ready` (DB-exercising) split fixing a real prior incident. EA's `web/app.py` was 2178 lines, 71 routes (23 reports, 19 plans, 6 auth, 5 drafts, 4 feedback, 2 each templates/sessions/query/plan-templates, 1 each status/perspectives/egeria/actions/health/root), with only `admin.py` (499 lines, self-contained `APIRouter()`) actually extracted. Unlike `admin.py`, most of the remaining routes lean on shared module-level state a straight per-route move would duplicate or fork — `_get_rag()`'s `_rag` singleton, `QueryRequest`/`FeedbackRequest` models, `_INTENT_META`/`_intent_meta()`, the report-catalog helpers (`_topic_for`, `_spec_supported_formats`, `_catalog_formats`, `_is_runnable_spec`, `_load_report_catalog`). **Slice 1 done:** extracted `advisor/web/shared.py` (the cross-cutting state above, so future slices have somewhere to import it from) and `advisor/web/auth.py` (6 routes, the cleanest boundary — no dependency on `shared.py` at all). `app.py` now 1814 lines. `advisor.web.app` still re-exports everything `shared.py` holds that app.py's own remaining routes use (`_get_rag`, `QueryRequest`, `FeedbackRequest`, `_intent_meta`, `_BROWSER_FORMATS`, `_catalog_formats`, `_load_report_catalog`, `_extended_feedback_path`, `_STATIC`) — `advisor.rag_system` has a live lazy import of `_intent_meta` from `advisor.web.app` specifically, verified still resolves. Verified: `ruff check --select F` clean on all three files; `tests/unit/test_login_required_middleware.py` (13 tests, exercises `/api/auth/*` and `/api/admin/*` through `TestClient`) unchanged and passing; full suite 315 passed (same as pre-change baseline, 4 pre-existing unrelated failures deselected). **Remaining slices, in rising order of size/risk:** feedback (4) → templates/sessions/query/plan-templates (2 each) → drafts (5) → plans (19) → reports (23, largest and most entangled with the catalog helpers — do last). |
